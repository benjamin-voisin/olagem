...
function sentence_generator(max_caractere)
	...
	while (not finished) do
		...
		if (string.len(text) + string.len(words[n]) < max_caractere - 4) then
...
function cadavres_exquis()
	local noms = {"Le cadavre", "Le chat", "Le chien", "La grenouille"}
	noms.taille = 4
	local adjectifs1 = {"exquis", "noir", "pouilleux", "gluant"}
	adjectifs1.taille = 4
	local verbes = {"boira", "mange", "aime", "capture"}
	verbes.taille = 4
	local complements = {"le vin", "un rat", "un os", "un moustique"}
	complements.taille = 4
	local adjectifs2 = {"nouveau", "gris", "moelleux", "agressif"}
	adjectifs2.taille = 4
	local phrase = noms[math.random(noms.taille)].." "..adjectifs1[math.random(adjectifs1.taille)].." "..verbes[math.random(verbes.taille)].." "..complements[math.random(complements.taille)].." "..adjectifs2[math.random(adjectifs2.taille)].."."
	return phrase
end
